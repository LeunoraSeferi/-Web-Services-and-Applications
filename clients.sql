...
-- Admin2 buys 2 Puma Running Shoes
INSERT INTO order_items (order_id, product_id, quantity, price)
VALUES (
  (SELECT id FROM orders WHERE client_id=(SELECT id FROM clients WHERE email='admin2@example.com') LIMIT 1),
  (SELECT id FROM products WHERE name='Puma Running Shoes' LIMIT 1),
  2,
  (SELECT price FROM products WHERE name='Puma Running Shoes' LIMIT 1)
);

-- Admin Test Client buys 1 H&M Winter Coat
INSERT INTO order_items (order_id, product_id, quantity, price)
VALUES (
  (SELECT id FROM orders WHERE client_id=(SELECT id FROM clients WHERE email='nora@example.com') LIMIT 1),
  (SELECT id FROM products WHERE name='H&M Winter Coat' LIMIT 1),
  1,
  (SELECT price FROM products WHERE name='H&M Winter Coat' LIMIT 1)
);

-- Admin2 buys 3 Nike Caps
INSERT INTO order_items (order_id, product_id, quantity, price)
VALUES (
  (SELECT id FROM orders WHERE client_id=(SELECT id FROM clients WHERE email='admin2@example.com') LIMIT 1),
  (SELECT id FROM products WHERE name='Nike Cap' LIMIT 1),
  3,
  (SELECT price FROM products WHERE name='Nike Cap' LIMIT 1)
);
